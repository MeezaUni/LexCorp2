...
async function main() {
  ...
  // ── Signers ──────────────────────────────────────────────────────────
  const [admin, user1, user2, auditor] = await hre.ethers.getSigners();
  console.log(`Admin (MANAGER):  ${admin.address}`);
  console.log(`User1:            ${user1.address}`);
  console.log(`User2:            ${user2.address}`);
  console.log(`Auditor:          ${auditor.address}\n`);

  const deployment = require("./contracts/deployments/localhost.json");
  ...
  const physicalTokenId = 501;
  const physicalSerial = "LEX-E2E-PHYSICAL-001";
  ...
  const digitalTokenId = 601;
  const digitalSerial = "LEX-E2E-DIGITAL-DOC-001";
  ...
  const user2DID = `did:ethr:13371:${user2.address.toLowerCase()}`;

  // 8a. Check initial access — user2 should NOT have access
  const checkBefore = await api("POST", "/api/assets/check-access", {
    token_id: digitalTokenId,
    requester_did: user2DID,
    caller_address: user2.address,
  });
  assert(checkBefore.data?.authorized === false, `User2 initially denied access (authorized: ${checkBefore.data?.authorized})`);

  // 8b. Grant READ access to user2 on digital asset
  ...
  // 8c. Check access again — user2 should now have READ
  const checkAfterGrant = await api("POST", "/api/assets/check-access", {
    token_id: digitalTokenId,
    requester_did: user2DID,
    caller_address: user2.address,
  });
  assert(checkAfterGrant.data?.authorized === true, `User2 now authorized (level: ${checkAfterGrant.data?.permission_level})`);
  ...
  // 8e. Revoke access
  const revokeAccessTx = await contract.connect(admin).revokeAssetAccess(digitalTokenId, user2DID);
  ...
  assert(checkAfterRevoke.data?.authorized === false, `User2 access revoked (authorized: ${checkAfterRevoke.data?.authorized})`);
  ...
  const burnTokenId = 701;
  const burnSerial = "LEX-E2E-BURN-TEST-001";
  ...
  const tempRes = await api("POST", "/api/users", {
    wallet_address: "0x4C9Ed15365a68A0B538bB2084d3bD8730a2d9e11", // random Hardhat address
